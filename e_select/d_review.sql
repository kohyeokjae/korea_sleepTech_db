...
SELECT 
    name,
    grade,
    (SELECT -- where 조건에서 조회된 회원의 총 구매 금액을 서브쿼리로 계산
            SUM(amount)
        FROM
            purchases
        WHERE
            member_id = `members`.member_id) AS total_amount
FROM
    members
WHERE
    member_id = (SELECT 
...
FROM
    `members`
ORDER BY name; -- 오름차순 asc은 생략 가능 (desc는 반드시 명시!)

# 2) 3번재 레코드부터 2개의 행 조회
SELECT * FROM `members`;
SELECT * FROM `members` LIMIT 2 OFFSET 2; -- 첫 행의 시작 번호가 0 (index 번호와 동일)
SELECT * FROM `members` LIMIT 2, 2; -- offset 시작번호는 생략 후 콤마 사용 가능
...
SELECT 
    gender, AVG(points) avg_points -- as 키워드 생략 가능
FROM
    `members`
GROUP BY gender;
